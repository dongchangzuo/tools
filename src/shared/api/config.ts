...
export const API_BASE_URL = ((): string => {
  if (import.meta.env.VITE_API_BASE_URL) {
    return trimTrailingSlash(import.meta.env.VITE_API_BASE_URL as string)
  }
  const host = (import.meta.env.VITE_API_HOST as string) ?? DEFAULT_API_HOST
  const path = (import.meta.env.VITE_API_PATH as string) ?? DEFAULT_API_PATH
