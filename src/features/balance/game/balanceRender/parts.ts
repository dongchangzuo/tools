...
export function drawEqualsSign(
  ...
  if (checkStatus === 'idle') {
    ...
    const eq = document.createElement('canvas').getContext('2d')!
    eq.font = ctx.font
    const eqW = eq.measureText('=').width

    ctx.save()
    ctx.translate(PIVOT_X, PIVOT_Y - 22)
    ctx.rotate(tiltRad * 0.25)
    ctx.fillText('=', 0, 0)
    ctx.restore()

    ctx.fillStyle = '#8a939d'
    ctx.font = '13px system-ui, sans-serif'
    ctx.fillText('??', PIVOT_X - eqW / 2 - 32, PIVOT_Y - 22)
    ctx.fillText('??', PIVOT_X + eqW / 2 + 10, PIVOT_Y - 22)
    return
  }
  ...
  if (checkStatus === 'imbalance') {
    drawCrackedEquals(ctx, Math.min(flashPhase / 1.8, 1))
  }
